            ...
            <div class="tab-content" style="height: 100%">
                <div class="tab-pane active" id="canhan">
                    <div class="profile_main_block p-4 bg-white">
                        <h4 style="display: flex; flex-direction: row; align-items: center"><img src="{{$sinhvien_static->anhthe}}" class="avatar mr-2" style="border-radius: 99px"/> Thêm tạm trú - {{$sinhvien_static->hodem . " " . $sinhvien_static->ten . " (" . $sinhvien_static->masv . ")"}}</h4>
                        <hr/>
                        <form action="{{route('ad.suasinhvien.themtamtrupost', ['masv' => $sinhvien_static->masv])}}" method="post" class="row">
                            {{ csrf_field() }}
                                        ...
                                        <div class="detail-content">
                                            <select name="namhoc_hocky" class="form-control rounded" required>
                                                ...
                                                @forelse($hocky_info as $key_hk => $item_hk)
                                                    <option value="{{$item_hk->id."|".$item_hk->hocky}}">Năm học {{$item_hk->nambatdau ."-". $item_hk->namketthuc}} HK {{$item_hk->hocky}}</option>
                                                @empty
                                ...
                                <div class="row">
                                    <div class="form-group">
                                        <div class="col-md-12">
                                            ...
                                            <a href="{{route('ad.suasinhvien.themtamtru', ['tamtru_id' => $tamtrukey, 'masv' => $sinhvien_static->masv])}}" class="btn btn-primary"><i class="fa fa-clock">&nbsp;</i>Lấy địa chỉ cũ</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
