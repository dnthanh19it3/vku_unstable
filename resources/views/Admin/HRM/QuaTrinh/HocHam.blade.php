            ...
            <div class="body">
                    ...
                    <div class="col-md-12">
                        <div class="btn-group" role="group" aria-label="Basic example">
                            <button class="btn btn-primary" onclick="addModal('#modal_themsua', '#form_themsua',
                                    '{{route('ad.hrm.hocham.them.post', ['ma_gv' => $giangvien->ma_gv])}}')"><i class="fa fa-plus-circle mr-2"></i>Thêm</button>
                                ...
                                <form class="modal-content" id="form_themsua" method="post" action="">
                                    ...
                                    <div class="modal-body">
                                        <div class="form-content">
                                            <input type="hidden" name="data[Bosunghocham][ma_gv]" id="ma_gv" value="{{$giangvien->ma_gv}}">
                                            ...
                                            <div class="form-row form-group">
                                                <label class="col-3 col-form-label">Tên học hàm <span class="warning"> *</span>:</label>
                                                <div class="col-9">
                                                    <select name="data[Bosunghocham][hocham_id]" id="hocham_id" id="selectChucDanhModal" required="" class="UserHochamId form-control rounded form-control rounded-sm">
                                                        <option value="">Chọn</option>
                                                        @forelse($hocham as $key => $value)
                                                            <option value="{{$value->key}}">{{$value->hocham}}</option>
                                                        @empty
                                                        @endforelse
                                                    </select>
                                                </div>
                                            ...
                                            <div class="form-row form-group">
                                                <label for="Bosunghochamnamcongnhan" class="col-3 col-form-label">Năm công nhận <span class="warning"> *</span>:</label>
                                                <div class="col-9">
                                                    <select name="data[Bosunghocham][namcongnhan]" id="namcongnhan" label="Năm tốt nghiệp:" required="required" class="namtotnghiep_qtbd form-control rounded form-control rounded-sm">
                                                        <option value=""></option>
                                                        @for($i = 1996; $i < 2026; $i++)
                                            ...
                                            <div class="form-row form-group">
                                                <label class="col-3 col-form-label">Ngành đào tạo <span class="warning"> *</span>:</label>
                                                <div class="col-9">
                                                    <select name="data[Bosunghocham][danhhieu_id]" id="danhhieu_id" id="selectDanhhieunhagiaoModal" required="" class="UserDanhhieuId form-control rounded form-control rounded-sm">
                                                        <option value="">Chọn</option>
                                                        @forelse($danhhieu as $key => $value)
                                                            <option value="{{$value->key}}">{{$value->danhhieu}}</option>
                                                        @empty
                                                        @endforelse
                                                    </select>
                                                </div>
                                            ...
                                            <div class="form-row form-group">
                                                <label for="Bosunghochamnamcongnhandanhhieu" class="col-3 col-form-label">Năm công nhận danh hiệu <span class="warning"> *</span>:</label>
                                                <div class="col-9">
                                                    <select name="data[Bosunghocham][namcongnhandanhhieu]" id="namcongnhandanhhieu" label="Năm tốt nghiệp:" required="required" class="namtotnghiep_qtbd form-control rounded form-control rounded-sm">
                                                        <option value=""></option>
                                                        @for($i = 1990; $i < 2026; $i++)
                                                            <option value="{{$i}}">{{$i}}</option>
                                                        @endfor
                ...
                <table class="table table-striped jambo_table bulk_action">
                    ...
                    <tr class="headings">
                        ...
                        <th class="column-title">Tên chức danh </th>
                        <th class="column-title">Năm công nhận chức danh </th>
                        <th class="column-title">Tên danh hiệu </th>
                        <th class="column-title">Năm công nhận danh hiệu</th>
                    ...
                    <tbody>
                        ...
                        @forelse($data as $key => $item)
                            <tr class="even pointer">
                                ...
                                <td class=" ">{{$i++}}</td>
                                <td class=" ">{{$item->hocham}}</td>
                                <td class=" ">{{$item->namcongnhan}}</td>
                                <td class=" ">{{$item->danhhieu}}</td>
                                <td class=" ">{{$item->namcongnhandanhhieu}}</td>
                                <td class=" last">
                                    <button class="btn btn-outline-blue btn-sm btn-primary text-white" onclick="editModal('#modal_themsua', '#form_themsua',
                                            '{{route('ad.hrm.hocham.sua.post', ['ma_gv' => $giangvien->ma_gv, 'id' => $item->id])}}',
                                            '{{route('ad.hrm.hocham.getdata', ['ma_gv' => $giangvien->ma_gv, 'id' => $item->id])}}')"><i class="fa fa-pencil"></i></button>
                                    <button class="btn btn-outline-blue btn-sm btn-danger text-white" onclick="deleteModal('#modal_xoa', '{{route('ad.hrm.hocham.xoa', ['ma_gv' => $giangvien->ma_gv, 'id' => $item->id])}}')"><i class="fa fa-times-circle"></i></button>
                                </td>
                            </tr>
                        @empty
                            <tr class="even pointer">
